"""PaddleOCR-VL document-pipeline adapter backed by MLX-VLM."""
...
class PaddleOcrVlAdapter:
    """Keep one PaddleOCR-VL document pipeline alive for all command pages."""
    ...
    def __init__(
        ...
        if max_concurrency is None:
            self._pipeline = PaddleOCRVL(
                ...
                vl_rec_server_url=mlx_server_url,
                vl_rec_api_model_name=mlx_model_name,
            )
        else:
            self._pipeline = PaddleOCRVL(
                vl_rec_backend="mlx-vlm-server",
